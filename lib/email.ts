...
export async function sendPasswordResetEmail(
  ...
  const from = process.env.EMAIL_FROM_ADDRESS || "onboarding@resend.dev";
  const subject = "Reset your DocHub password";
  const html = `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
      <h2>Reset your password</h2>
      <p>Click the button below to reset your DocHub password. This link expires in 1 hour.</p>
      <p>
        <a
          href="${input.resetUrl}"
          style="display: inline-block; padding: 12px 24px; background: #111; color: #fff; text-decoration: none; border-radius: 6px;"
  ...
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.log("[EMAIL FALLBACK] Password reset email would be sent:");
    console.log(`  To: ${input.to}`);
    console.log(`  From: ${from}`);
    console.log(`  Subject: ${subject}`);
